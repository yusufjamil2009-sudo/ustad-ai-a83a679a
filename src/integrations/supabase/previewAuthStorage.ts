...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      // timer is intentionally assigned AFTER the listeners are wired up (the
      // timeout must not start before the request is posted); prefer-const
      // cannot see that ordering, so keep the deferred assignment.
