...
--
-- EXTENDS the existing coin system; it does NOT replace it. `ustad_coin_ledger`
-- (Part 1) stays the append-only record of every coin movement and remains the
-- reconstructable source of truth. This migration adds:
--   * a permanent wallet aggregate (one per guest) kept in lockstep with the
--     ledger inside the SAME database transaction,
--   * the richer per-transaction fields Part 7 requires,
--   * the shop catalogue, purchases and ownership,
--   * atomic SQL functions so a coin movement or a purchase either fully
--     happens or fully does not.
--
-- USTAD Coins are VIRTUAL in-app coins. Not money, not INR, not USD.

/* ------------------------------------------------------------------ */
/* 1. Ledger: richer transaction record (additive columns only)        */
/* ------------------------------------------------------------------ */
...
-- Direction is derived from the signed amount, so it can never disagree.
...
/* ------------------------------------------------------------------ */
/* 2. Permanent wallet — ONE per guest, never reset                    */
/* ------------------------------------------------------------------ */
...
create table if not exists public.ustad_wallets (
  ...
  -- ONE ACTIVE WALLET PER GUEST. A second wallet cannot be created by accident.
  constraint ustad_wallets_guest_unique unique (guest_id),
  -- A wallet can never go negative, at the database level.
...
-- Backfill wallets from the EXISTING ledger so no historic coin is lost.
...
create table if not exists public.ustad_purchases (
  ...
  -- A permanent item can be owned only once → no double charge, ever.
...
/* ------------------------------------------------------------------ */
/* 5. ATOMIC coin movement                                             */
/* ------------------------------------------------------------------ */

-- Applies one coin movement: locks the wallet row, records the ledger entry
-- with balance_before/balance_after, and updates the wallet — all in ONE
-- transaction. Idempotent on (guest_id, source, ref_id): replaying a request
-- returns the original transaction and never moves coins twice.
...
begin
  ...
  -- Replay protection: the same (guest, source, ref) is one logical movement.
  ...
  -- The guest must exist; the wallet is created once and then reused forever.
  ...
  -- Lock the wallet: concurrent requests serialize here, so two tabs spending
  -- at the same instant can never both pass the balance check.
...
-- Buys one shop item. The price comes from the SERVER catalogue — the caller
-- cannot supply it. Deduction, purchase record and ownership all happen in one
-- transaction, so "coins gone but no item" and "item granted but no coins
-- deducted" are both impossible.
...
begin
  ...
  -- Already owned → return the original purchase, charge nothing.
  select * into v_existing
  ...
  -- Authoritative price, debited atomically. Raises if funds are short, which
  -- rolls back this whole function: no purchase row, no coin movement.
...
/* ------------------------------------------------------------------ */
/* 7. Crorepati reward ladder — Part 7 exact values                    */
/* ------------------------------------------------------------------ */

-- Server-side configuration. The frontend cannot change these.
...
values
  ...
  -- Cosmetic badges (decorative; NOT verified achievements)
  ...
  -- Classroom themes (presentation only; the Classroom engine is unchanged)
  ...
  -- Board themes (presentation layer only; the Board engine is unchanged)
  ...
  -- Teacher presentation themes (visuals only; AI ability is unchanged)
  ...
  -- Tournament cosmetics (decorative; they never create a victory)
  ...
  -- Feature unlocks (presentation/customization only — never pay-to-win)
...
-- No anon policies: the browser can never read or write these directly.
-- All access goes through server functions that verify guest ownership.
